...
IntPtr lpSecurityAttributes
		...
		public VHDXTool(string isoFilePath)
		{
			...
			try
			{
				using (FileStream fileStream = File.Open(IsoFilePath, FileMode.Open, FileAccess.Read, FileShare.Read)) { }
			}
			catch (IOException)
			{
				string ConfigDir = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), "BigBoxProfile");
				string CmdMountFile = Path.Combine(ConfigDir, "unmountcmd.txt");
				File.WriteAllText(CmdMountFile, IsoFilePath);
				BigBoxUtils.ExecuteTask("BigBoxPS3IsoUnmount");
			}
		...
		{
			MessageBox.Show("Debug PS3 VHDX ON");
			...
			BigBoxUtils.ExecuteTask("BigBoxPS3IsoMount");
		...
		public static void EnableGameDirWatcher(string PathGame, string PathRap)
		{
			...
			watcher.Created += new FileSystemEventHandler((sender, args) =>
			{
				...
				{
					if (!intermediateDirectory.Contains("locks")) GameDirChanged.Add(intermediateDirectory);
				}
			});

			watcher.Renamed += new RenamedEventHandler((sender, args) =>
			{
				string intermediateDirectory = VHDXTool.GetIntermediateDirectory(PathGame, args.FullPath);
				Console.WriteLine(intermediateDirectory);
				if (!GameDirChanged.Contains(intermediateDirectory))
		...
		public static string FindEboot(string sourceDir, int id = 0)
		{
			...
			//var files = Directory.GetFiles(sourceDir, "EBOOT.BIN", SearchOption.AllDirectories);
			var files = BigBoxUtils.FindAccessableFiles(sourceDir, "EBOOT.BIN", true);
		...
		public static void CreateVHDX(string rootPath, string target,string isofile="")
		{
			...
			{
				GameDirChanged.Remove(dir);
			}
			if (GameDirChanged.Count()==0) {
				throw new Exception("No change in game dir");
				return;
			}

			if (isofile != "")
			{
				FileInfo isoInfo = new FileInfo(isofile);
				long sizeSource = isoInfo.Length;
				long sizeDest = (long)Math.Round(((double)sizeSource * 1.5) / 1024 / 1024) + 30;
				totalSizeNeeded = sizeDest;
			}
		...
		public static string GetIntermediateDirectory(string directoryRoot, string directoryElement)
		{
			...
			string relativePath = BigBoxUtils.MakeRelativePath(directoryRoot, directoryElement);
		...
		private static string[] FilterDirectories(string[] directories)
		{
			return directories.Where(dir =>
			{
				...
				bool containsLock = Path.GetFileName(dir).ToLower().Contains("locks");
